...
import { BookOpen, Users, GraduationCap, Baby, Library, X, ArrowLeft, Heart } from 'lucide-react';
import { ACTIVITIES, EXTERNAL_LINKS } from '../constants';
...
const Activities: React.FC = () => {
  ...
  const getIcon = (id: string) => {
    switch(id) {
      ...
      case 'yeshiva': return <BookOpen className="w-5 h-5 md:w-6 md:h-6" />;
      case 'women': return <Users className="w-5 h-5 md:w-6 md:h-6" />;
      case 'kindergarten': return <Baby className="w-5 h-5 md:w-6 md:h-6" />;
          ...
          {ACTIVITIES.map((activity, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 z-0">
                ...
                />
                {/* Darker overlay on mobile by default to make text readable, lighter on desktop until hover */}
              ...
              <div className="absolute inset-0 z-10 flex flex-col justify-end p-6 md:p-8 text-white">
                <div className="transform translate-y-0 md:translate-y-4 md:group-hover:translate-y-0 transition-transform duration-500">
                  ...
                  {/* Icon - Always visible on mobile, fade in on desktop hover */}
                  ...
                  <h3 className="text-2xl md:text-2xl font-serif font-bold mb-2 group-hover:text-gold-100 transition-colors shadow-sm">
                    {activity.title}
                  </h3>
                  
                  {/* Description - Hidden on mobile to save space, visible on desktop hover */}
              ...
              >
                <div className="bg-white shadow-luxury relative rounded-sm overflow-hidden">
                   ...
                   <div className="flex flex-col lg:flex-row min-h-[500px] md:min-h-[600px]">
                      {/* Content Side */}
                      <div className="p-8 md:p-10 lg:p-20 lg:w-1/2 flex flex-col justify-center bg-white order-2 lg:order-1">
                        <div className="flex items-center gap-3 text-gold-600 mb-4 md:mb-6">
                          {getIcon(selectedActivity.id)}
                          <span className="font-bold tracking-widest uppercase text-sm">הרחבה</span>
                        </div>
                        ...
                        >
                          <span className="tracking-widest font-bold text-base md:text-lg">תרום לפעילות זו</span>
                      ...
                      <div className="h-64 md:h-auto lg:w-1/2 bg-gray-100 relative order-1 lg:order-2">
                        ...
                        />
                        <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent flex items-end p-4 md:p-10">
                            {/* Thumbnails */}
                            <div className="flex gap-2 md:gap-4 overflow-x-auto pb-2 scrollbar-hide">
                                {selectedActivity.gallery.map((img, idx) => (
                                <div key={idx} className="w-20 h-14 md:w-32 md:h-20 flex-shrink-0 border-2 border-white/30 rounded-sm overflow-hidden cursor-pointer hover:border-gold-400 transition-colors">
